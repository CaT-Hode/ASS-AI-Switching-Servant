...
- 额外识别 Kimi Code 新 / 旧版、ZCode CLI / Desktop 与 Antigravity CLI。Kimi / ZCode 支持按供应商直写原生配置、同步及撤回；可发起原生登录，文件型 OAuth 自动加密记录并在账户卡片切换，不改变模型接入范围；本地 Wire / SQLite 用量接入首页统计。Antigravity 仅识别 CLI 配置与明确启用的 Gemini API。具体格式与尚未实现的能力见 [新增客户端支持边界](docs/ADDITIONAL-HARNESSES.md)。
