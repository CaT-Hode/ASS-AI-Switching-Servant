...
- 额外识别 Kimi Code 新 / 旧版、ZCode 与 Antigravity CLI。Kimi 支持按供应商直写 TOML、同步及撤回，保留原有 OAuth；Kimi / ZCode 的本机账户和声明模型可查看。Antigravity 仅识别 CLI 配置与明确启用的 Gemini API。新增客户端的 OAuth 换号与用量采集仍待适配，详见 [新增客户端支持边界](docs/ADDITIONAL-HARNESSES.md)。
...
全局“接入 Codex”按钮已移除。Codex / Claude / OpenCode / pi / DSH / Kimi 各有接入开关，点击后显示简短警告和“确定 / 取消”，不展示分步菜单或 PID 清单。需要刷新桌面进程的客户端额外提供“立即重启”，每次默认不勾选。默认焦点在取消；取消、Escape 或默认回车都不会更改接入。
...
模型在“供应商与模型”统一管理。客户端的“供应商接入”按供应商整组开关，默认纳入其中已启用、有凭据且协议兼容的模型；展开只读查看模型，不再提供逐模型勾选、默认模型或“启动模型”。模型选择交给原生客户端。DSH / OpenCode / pi / Kimi 直写原生配置；Codex 通过 Responses 路由保留官方订阅目录并合并第三方模型；Claude 仅纳入 Messages 协议，官方账户窗口不混入 API 路由。
