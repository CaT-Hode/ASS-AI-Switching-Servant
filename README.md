...
- 每个客户端独立的接入开关；二级菜单预览影响，确认后恢复注入、按需结束 ASS 窗口与停止服务。
...
全局“接入 Codex”按钮已移除。五种客户端各有接入开关，点击后先选择操作，再查看配置、窗口和共享服务的影响，勾选确认后执行。取消、Escape 和第一层“继续”都不会更改接入。

- **开启**：启用该客户端的本地路由，API 注入在下次从 ASS 启动时生成。Codex 还会更新 App 配置；ASS 不擅自重启 App。其他客户端的原生 OAuth 保持官方直连。
- **安全断开（默认）**：先自行退出客户端。只要仍有已登记窗口、无法确认身份的窗口或进行中请求，就拒绝断开。
- **结束 ASS 窗口后断开**：第二层明确列出窗口和 PID。只结束经身份校验的 ASS 启动窗口及已确认子进程；不按进程名批量结束程序。即使选此项，仍有进行中请求时也会拒绝操作。
...
- **停止全部 / 安全退出**：位于客户端页及托盘，同样经过二级确认，不再一键中断路由。
