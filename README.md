...
- 原生模型列表也可点击闪电逐模型检测，使用对应原生凭据直连，不开启代理或重启客户端；已测试的闪电填充并显示上次测试时间，结果按客户端、账户目录、供应商和模型隔离持久化。凭据或连接配置变化后旧结果失效；仅收到 HTTP 200 不算通过。
- 自动检测各已支持客户端的本机 OAuth / API 凭据，按账户与供应商在右侧显示卡片。支持默认目录、环境变量目录与手动指定目录。
- 额外识别 Kimi Code 新 / 旧版与 ZCode CLI / Desktop。两者支持按供应商直写原生配置、同步及撤回；可发起原生登录，文件型 OAuth 自动加密记录并在账户卡片切换，不改变模型接入范围；本地 Wire / SQLite 用量接入首页统计。ZCode Start Plan 的运行时模型白名单会按当前账户聚合到本机目录，查询失败保留同一账户上次成功结果。具体格式与尚未实现的能力见 [新增客户端支持边界](docs/ADDITIONAL-HARNESSES.md)。
...
OpenAI / Anthropic OAuth 显示接口实际返回的剩余额度窗口；不把主窗口固定解释成 5h，也不把缺失值当作 0。当前与已保存的 OAuth 账户可独立查询额度，不改变客户端登录。Kimi Code 另支持用户资料、编程额度及加量包余额；ZCode 分别读取 Start Plan、个人 / Team Coding Plan 和 MCP 额度，组织 / 项目与计量单位不混合。Coding Plan 也支持 CLI 账户；Start Plan 仍需读取 Desktop 真实版本。DeepSeek 等 API 按原币种显示可用余额。供应商卡片打开时及可见期间每 5 分钟只读刷新，可手动刷新；查询失败保留带时间的上次结果。具体来源、缓存和接口边界见 [额度与图标](docs/SUPPLIER-CARDS.md)及[新增客户端支持边界](docs/ADDITIONAL-HARNESSES.md)。
