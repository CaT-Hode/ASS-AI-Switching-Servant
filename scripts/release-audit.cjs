...
const forbidden =
  /(?:^|\/)(?:auth\.json|\.credentials\.(?:json|yaml)|preferences\.json|settings\.json|clients\.json|native-accounts\.json|account-info\.enc\.json|updates\.json|connections\.json|route-injections\.json|client-processes\.json|codex-attachment\.json|requests\.jsonl(?:\.previous)?|[^/]+\.aimami-relay\.json)$/i;
const tokenShape =
